...
def load_data():
    ...
    try:
        ...
        stockprice = pd.read_excel(
            "process_data/StockPriceValue_complete.xlsx", 
            header=0, 
            index_col=0, 
            sheet_name="Price", 
            parse_dates=['Date']
        ...
        shares = pd.read_excel(
            "process_data/Shares_complete.xlsx", 
            header=0, 
            index_col=0, 
            sheet_name="shares"
        )
        ...
        daily_monitor = pd.read_excel(
            "process_data/每日监控数据_complete.xlsx",
            sheet_name="chart",
            header=0,  # 第1行作为列标题
        ...
        try:
            funds_value = pd.read_excel(
                "process_data/FundsValue_complete.xlsx",
                sheet_name="Price",
                header=0,
                index_col=0,
                parse_dates=True
...
def calculate_cumulative_returns(prices, periods=[30, 90, 180, 252]):
    ...
    try:
        if prices.empty:
            st.warning("价格数据为空，无法计算累计收益率")
            return {}
        
        if len(prices) < max(periods):
            st.warning(f"价格数据长度不足，需要至少{max(periods)}天数据，当前只有{len(prices)}天")
            return {}
        ...
        for period in periods:
            if len(prices) > period:
                try:
                    ...
                    # 检查基准价格是否有效
                    valid_start_prices = start_prices[(pd.notna(start_prices)) & (start_prices != 0)]
                    valid_end_prices = end_prices[pd.notna(end_prices)]
                    
                    if len(valid_start_prices) > 0 and len(valid_end_prices) > 0:
                        # 只计算有效价格的收益率
                        common_tickers = valid_start_prices.index.intersection(valid_end_prices.index)
                        if len(common_tickers) > 0:
                            returns = pd.Series(index=common_tickers, dtype=float)
                            for ticker in common_tickers:
                                if valid_start_prices[ticker] != 0:
                                    returns[ticker] = (valid_end_prices[ticker] - valid_start_prices[ticker]) / valid_start_prices[ticker] * 100
                            
                            if not returns.empty and returns.notna().any():
                                cumulative_returns[f'{period}d'] = returns
                            else:
                                st.warning(f"⚠️ {period}天累计收益率计算结果为空")
                        else:
                            st.warning(f"⚠️ {period}天期间没有共同的Ticker")
                    else:
                        st.warning(f"⚠️ {period}天期间基准价格或当前价格无效")
                        
                except Exception as period_error:
                    st.error(f"{period}天累计收益率计算错误: {period_error}")
                    continue
        
        if not cumulative_returns:
            st.warning("所有期间的累计收益率计算都失败")
...
def calculate_specific_period_returns(prices):
    ...
    try:
        ...
        if not specific_returns:
            st.warning("特定期间收益率计算结果为空")
...
def plot_cumulative_returns_since_2025(prices, selected_tickers=None):
    ...
    try:
        ...
        # 如果基准日期价格无效，寻找最近的有效交易日
        if base_prices.isna().all() or (base_prices <= 0).all():
            # 寻找最近的有效交易日
            valid_start_idx = None
            for date in prices_since_start.index:
                temp_prices = prices_since_start.loc[date]
                if temp_prices.notna().any() and (temp_prices > 0).any():
                    valid_start_idx = date
                    break
            
            if valid_start_idx is not None:
                base_prices = prices_since_start.loc[valid_start_idx]
                start_idx = valid_start_idx  # 更新起始日期
            else:
                st.error("❌ 无法找到有效的基准日期")
                return go.Figure()
        ...
        for ticker in available_tickers:
            if ticker in prices_since_start.columns:
                ...
                # 如果基准价格仍然无效，尝试寻找该Ticker的第一个有效价格
                if pd.isna(base_price) or base_price <= 0:
                    ...
                    if len(valid_prices) > 1:
                        ...
                        # 计算从第一个有效价格开始的收益率
                        returns = (valid_prices - first_valid_price) / first_valid_price * 100
                        cumulative_returns[ticker] = returns
                    else:
                        st.warning(f"⚠️ {ticker}: 没有足够的有效价格数据")
                else:
                    # 计算每日累计收益率
...
def plot_funds_cumulative_returns_since_2025(funds_value_data, selected_funds=None):
    ...
    try:
        ...
        if base_prices.isna().all() or (base_prices <= 0).all():
            # 寻找最近的有效交易日
            valid_start_idx = None
            for date in funds_since_start.index:
                temp_prices = funds_since_start.loc[date]
                if temp_prices.notna().any() and (temp_prices > 0).any():
                    valid_start_idx = date
                    break
            
            if valid_start_idx is not None:
                base_prices = funds_since_start.loc[valid_start_idx]
                start_idx = valid_start_idx  # 更新起始日期
            else:
                st.error("❌ 无法找到有效的基准日期")
                return go.Figure()
            ...
            if fund in funds_since_start.columns:
                ...
                if pd.isna(base_price) or base_price <= 0:
                    ...
                    if len(valid_prices) > 1:
                        ...
                        st.warning(f"⚠️ {fund}: 没有足够的有效价格数据")
                else:
                    ...
                    if len(valid_prices) > 1:
                        ...
                        st.warning(f"⚠️ {fund}: 有效数据点不足")
        
        if not cumulative_returns:
            st.warning("无法计算累计收益率")
            st.error("可能的原因：")
            st.error("1. 所有基金的基准价格都无效")
            st.error("2. 数据质量问题（价格<=0或NaN）")
            st.error("3. 有效数据点不足")
            st.error("4. 日期范围设置不当")
            return go.Figure()
